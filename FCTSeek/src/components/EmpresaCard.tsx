  ...
  return (
    <TouchableOpacity style={styles.card} onPress={onPress}>
      ...
      <View style={styles.cursos}>
        {empresa.cursos.slice(0, 3).map((curso) => (
          <Text key={curso.id} style={styles.cursoTag}>{curso.siglas}</Text>
        ))}
        {empresa.cursos.length > 3 && (
          <Text style={styles.cursoTag}>+{empresa.cursos.length - 3}</Text>
        )}
      </View>
    </TouchableOpacity>
  );
